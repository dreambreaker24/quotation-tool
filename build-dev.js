...
// ── 12. updateCat：名稱改成牌價表分類時自動帶入材料說明 ──
out = out.replace(
...
// ── 13. renderSubInput：複製按鈕 + 算式數量輸入 ──
// 13a. 複製按鈕（替換原本刪除按鈕，改成複製+刪除兩顆）
out = out.replace(
...
// 13b. 算式數量輸入（type=number → type=text，支援 5+3 等算式）
out = out.replace(
...
// ── 13c. spec 欄 input → 自動展開 textarea ──
out = out.replace(
...
// ── V4. 大項標題列：字體放大加粗 + 寬 padding（移除金色左線）──
out = out.replace(
...
// ── W7. 合約條款可編輯 ──
// CSS：列印時顯示 ol、隱藏 textarea（textarea 已有 .no-print class）
out = out.replace('\n  </style>', function() {
    return '\n    @media print { #prev-contract-ol { display: block !important; } }\n  </style>';
});

// HTML：靜態 ol → 可編輯 textarea + 列印用 ol
out = out.replace(
    '        <ol style="padding:4px 10px 5px 26px; margin:0;">\n' +
...
// ── A1. newQuote：自動帶入合約編號（NS/PO-年份-流水號）──
out = out.replace(
    ...
    function() {
        return (
            ...
            `      var _pat = 'q_' + _pfx + '-' + _yr + '-';\n` +
            `      var _seqs = Object.keys(localStorage).filter(function(k){ return k.startsWith(_pat); })\n` +
            `        .map(function(k){ var m = k.match(/-(\\d+)$/); return m ? parseInt(m[1]) : 0; });\n` +
            `      var _next = _seqs.length > 0 ? Math.max.apply(null, _seqs) + 1 : 1;\n` +
            `      var _no = _pfx + '-' + _yr + '-' + String(_next).padStart(3, '0');\n` +
            `      document.getElementById('in-contract-no').value = _no;\n` +
...
// ── P1. 第一頁大項 qty 顯示 fallback：0 → 1（避免 NaN 測試殘留）──
out = out.replace(
...
// ── Bug1. updateCat qty NaN guard（已在步驟12加入，此處補 updateSubItem）──
out = out.replace(
...
// ── Bug2a. saveQuote：DEV 模式改走 localStorage ──
out = out.replace(
...
// ── Bug2b. saveQuote catch：識別 DEV sentinel ──
out = out.replace(
    `    } catch (e) {
...
// ── Bug2c. liveSearch：DEV 模式改搜 localStorage ──
out = out.replace(
    `    _searchDebounce = setTimeout(async () => {
      try {
        const snap = await db.collection('quotations').orderBy('updatedAt', 'desc').limit(500).get();
        const keyword = val.toLowerCase();
        _searchMatches = snap.docs.map(d => d.data())
          .filter(q => (q.contractNo && q.contractNo.toLowerCase().includes(keyword)) ||
                       (q.clientName && q.clientName.toLowerCase().includes(keyword)) ||
                       (q.projectName && q.projectName.toLowerCase().includes(keyword)));`,
    function() {
        return (
            `    _searchDebounce = setTimeout(async () => {\n` +
            `      try {\n` +
            `        // [DEV] 搜尋 localStorage\n` +
            `        const keyword = val.toLowerCase();\n` +
            `        const _allK = Object.keys(localStorage).filter(function(k){ return k.startsWith('q_'); });\n` +
            `        _searchMatches = _allK.map(function(k){ try{ return JSON.parse(localStorage.getItem(k)); }catch(e){ return null; } }).filter(Boolean)\n` +
            `          .filter(q => (q.contractNo && q.contractNo.toLowerCase().includes(keyword)) ||\n` +
            `                       (q.clientName && q.clientName.toLowerCase().includes(keyword)) ||\n` +
            `                       (q.project && q.project.toLowerCase().includes(keyword)));`
        );
    }
);

// ── Bug2d. DEV 登入後從 localStorage 載入歷史清單 ──
out = out.replace(
    `      document.getElementById('in-contract-terms').value = DEFAULT_CONTRACT_TERMS;\n      renderAll();\n    }`,
    function() {
        return (
...
// ── W8. savePDF 第一頁縮放修正 ──
// 原版：canvas1 全高截圖後直接 addImage，並用 Math.min(..., 297) 裁底
// 問題：內容超出 A4 時，印章/簽名會被截掉
// 修法：偵測 __pfit 的 zoom 比例，先把 canvas 縮小，再放進 PDF
out = out.replace(
    "      const canvas1 = await html2canvas(page1, captureOpts);\n      const page1H = Math.min(canvas1.height / canvas1.width * 210, 297);",
    function() {
        return (
            "      const canvas1 = await html2canvas(page1, captureOpts);\n" +
            "      // 若第一頁超出 A4，依 updatePrintZoom 縮放比例縮小 canvas，確保印章/簽名不被截\n" +
...
fs.writeFileSync('C:/AI助理 Claude/quotation-dev.html', out, 'utf8');
...
const checks = [
  ...
  ['DEV 登入繞過', out.includes('[DEV 測試模式]')],
  ['兩顆按鈕', out.includes('從牌價表選')],
  ['垃圾清運在牌價表', out.includes('"垃圾清運"')],
  ['工程中廢棄物清理在牌價表', out.includes('"工程中廢棄物清理"')],
  ['getCatNote 函數', out.includes('function getCatNote(')],
  ['renderPage2 注意事項用 cat.note', out.includes('var _note = cat.note')],
  ...
  ['Modal 記住分類', out.includes('_priceCatFilter')],
  ['Modal 自動對應分類', out.includes('dbMatch')],
  ['note 折疊區 HTML', out.includes('note-toggle-')],
  ...
  ['renderPage2 用 cat.note', out.includes('var _note = cat.note')],
  ['無 garbageEnabled', !out.includes('garbageEnabled')],
  ['無垃圾勾選框', !out.includes('id="in-garbage"')],
  ['刪除確認', out.includes('confirmRemoveCat(')],
  ...
  ['spec textarea', out.includes('scrollHeight')],
