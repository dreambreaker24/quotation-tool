  ...
  <div class="bg-white rounded-2xl shadow-md p-6">
    ...
    <div v-if="showForm" class="bg-gray-50 rounded-xl p-4 mb-5">
      <div class="grid grid-cols-2 gap-3">
        ...
        <div>
          <label class="text-xs text-gray-500 mb-1 block">生日（薪資單用，年份無意義）</label>
          <input v-model="form.birthDate" type="date" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-1">
        </div>
      </div>
    ...
    <table class="w-full text-sm">
      <thead>
        <tr class="bg-gray-100">
          ...
          <th class="text-left px-4 py-2.5 text-gray-600 font-semibold text-xs">生日</th>
      ...
      <tbody>
        <tr v-for="u in users" :key="u.id" class="border-t border-gray-100 hover:bg-amber-50/40 transition-colors">
          ...
          <td class="px-4 py-2.5 text-gray-500 text-xs">
            <template v-if="editingBirthDateId === u.id">
              <input v-model="birthDateValue" type="date"
                class="text-xs border border-gray-200 rounded px-2 py-1 focus:outline-none focus:ring-1"
                @keyup.enter="saveBirthDate(u.id)" @keyup.escape="editingBirthDateId = null">
              <button @click="saveBirthDate(u.id)" class="text-xs text-white px-2 py-1 rounded ml-1" style="background:#1e2533">確認</button>
              <button @click="editingBirthDateId = null" class="text-xs text-gray-400 ml-1">取消</button>
            </template>
            <template v-else>
              <span @click="startBirthDateEdit(u)" class="cursor-pointer hover:text-gray-700">{{ u.birthDate || '—' }}</span>
            </template>
          </td>
        ...
        <tr v-if="users.length === 0">
          <td colspan="10" class="px-4 py-6 text-center text-gray-400 text-xs">尚無帳號資料</td>
        </tr>
      </tbody>
    </table>
...
const form = ref({ name: '', fullName: '', email: '', role: 'employee', companyId: 'south', job: '', salary: 0, hireDate: '', birthDate: '' })
const renamingId = ref(null)
const renameValue = ref('')
const editingHireDateId = ref(null)
const hireDateValue = ref('')
const editingBirthDateId = ref(null)
...
async function createUser() {
    ...
    form.value = { name: '', fullName: '', email: '', role: 'employee', companyId: 'south', job: '', salary: 0, hireDate: '', birthDate: '' }
...
function startBirthDateEdit(u) {
    editingBirthDateId.value = u.id
    birthDateValue.value = u.birthDate || ''
}

async function saveBirthDate(id) {
    const date = birthDateValue.value
    if (!date) { editingBirthDateId.value = null; return }
    await updateDoc(doc(db, 'users', id), { birthDate: date })
    const target = users.value.find(u => u.id === id)
    if (target) target.birthDate = date
    editingBirthDateId.value = null
}
