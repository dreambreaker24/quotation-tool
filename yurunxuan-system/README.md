...
- 計畫四B-2（毛利分析＋即將到期成品提醒）：已完成，包含主檔資料新增毛利分析分頁（三款飲品並排比較單瓶成本與三層售價毛利率，無進貨資料時顯示「成本未知」而非誤導的0元毛利）、首頁與庫存總覽新增即將到期成品提醒卡（含已過期未報廢批次，庫存總覽該卡片員工也看得到）

Phase 1（骨架＋主檔＋進銷存＋收支儀表板）與計畫四（成品庫存、銷售品項化、毛利分析、效期提醒）已全部完成。後續是 Phase 2：LINE 通知（補貨提醒＋客戶訂單提醒），需要另外規劃 LINE Messaging API 串接方式。
