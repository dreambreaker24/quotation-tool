...
- 計畫四B-1（銷售登記品項化＋FIFO扣成品庫存）：已完成，包含收入登記改為選飲品/方案/組數多筆列表、送出時自動用先進先出扣減成品庫存批次，多品項若任一款庫存不足整筆登記失敗回滾

Phase 1（骨架＋主檔＋進銷存＋收支儀表板）已全部完成。四B-2（毛利分析畫面、即將到期成品提醒UI）尚未開始。後續是 Phase 2：LINE 通知（補貨提醒＋客戶訂單提醒），需要另外規劃 LINE Messaging API 串接方式。
