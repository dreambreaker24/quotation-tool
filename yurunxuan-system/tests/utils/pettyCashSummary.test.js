...
describe('calcBalance', () => {
  ...
  it('型別不是 topup 或 expense 時忽略，不影響餘額', () => {
    const entries = [
      { type: 'topup', amount: 10000 },
      { type: 'other', amount: 99999 },
    ]
    expect(calcBalance(entries)).toBe(10000)
  })
})
...
describe('buildPettyCashExportRows', () => {
  ...
  it('該月完全沒有紀錄時，只剩合計那一行，撥入/支出都是 0', () => {
    const rows = buildPettyCashExportRows([], '2026-09')
    expect(rows).toEqual([
      { '日期': '──', '類型': '合計', '分類': '', '金額': 0, '用途說明': '撥入 0／支出 0', '憑證': '' },
    ])
  })
})
