    ...
    async function addWasteLog({ date, type, materialId, drinkId, qty, reason, recordedBy, recordedByUid }) {
        ...
        if (type === 'material') {
            return runTransaction(db, async (transaction) => {
                ...
                transaction.set(logRef, { date, materialId, qty: validQty, reason, recordedBy, recordedByUid, createdAt: serverTimestamp() })
                return logRef.id
            })
        }
        return addDoc(collection(db, 'wasteLogs'), { date, drinkId, qty: validQty, reason, recordedBy, recordedByUid, createdAt: serverTimestamp() })
    }
