        ...
        <div class="bg-white rounded-2xl shadow-md p-5 mb-4">
            ...
            <div class="flex flex-col gap-1">
                <div v-for="m in materialsStore.materials" :key="m.id"
                    ...
                    :class="isLow(m) ? 'border-red-200 bg-red-50/60' : 'border-gray-100 bg-gray-50/50'">
                    <span class="flex-1 font-medium text-gray-800">{{ m.name }}<span class="ml-1.5 text-[10px] text-gray-400">{{ m.category }}</span></span>
                    <span class="font-medium" :class="isLow(m) ? 'text-red-500' : 'text-gray-600'">
                        {{ m.currentStock ?? 0 }} {{ m.unit }}
                    </span>
                    <span v-if="isLow(m)" class="text-[10px] px-1.5 py-0.5 rounded-full bg-red-100 text-red-600">低於安全庫存</span>
                </div>
            </div>
        </div>
...
function isLow(m) {
    return (m.safetyStock ?? 0) > 0 && (m.currentStock ?? 0) < m.safetyStock
}
